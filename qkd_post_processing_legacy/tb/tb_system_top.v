...
module tb_system_top;
    ...
    reg [1:0] code_rate = 2'b10; // Test Rate 3/4 ban dau
    ...
    // Lưu dữ liệu mô phỏng cho 10 blocks
    reg [LLR_W-1:0] llr_mem [0:LDPC_BLOCK*10-1];
    reg [0:0] syn_mem [0:1152*10-1];
    reg [0:0] expected_mem [0:LDPC_BLOCK*10-1];

    integer i, j, k;
    integer err_count;
    integer block_idx;
    integer err_counts_per_block [0:9];
    reg status_per_block [0:9];
    
    integer j_hash;
    integer err_count_hash;
    always @(posedge clk) begin
        if (dut.u_ldpc_core.state == 9) begin // WAIT_FOR_HASH
            err_count_hash = 0;
            for(j_hash = 0; j_hash < LDPC_BLOCK; j_hash = j_hash + 1) begin
                if (dut.ldpc_res[j_hash] !== expected_mem[block_idx * 2304 + j_hash]) begin
                    err_count_hash = err_count_hash + 1;
                end
            end
            
            if (err_count_hash == 0) begin
                hash_ok <= 1'b1;
                hash_fail <= 1'b0;
            end else begin
                hash_ok <= 1'b0;
    ...
    initial begin
        ...
        $display("Starting AXI-Stream Data Transfer for 10 Continuous Blocks...");
        
        for (block_idx = 0; block_idx < 10; block_idx = block_idx + 1) begin
            $display("=================================================");
            ...
            begin : VERIFY_BLOCK
                ...
                for(j = 0; j < LDPC_BLOCK; j = j + 1) begin
                    if (dut.ldpc_res[j] !== expected_mem[block_idx * 2304 + j]) begin
                        err_count = err_count + 1;
                        if (err_count <= 10) $display("Mismatch at bit %0d: Expected %b, Got %b", j, expected_mem[block_idx * 2304 + j], dut.ldpc_res[j]);
                    end
                end
