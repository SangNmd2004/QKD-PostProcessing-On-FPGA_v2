...
module core_partially_parallel #(
    ...
    localparam IDLE = 0, LOAD_LLR = 1, LAYER_READ = 2, LAYER_CALC = 3, LAYER_WRITE = 4, CHECK = 5, WAIT_FOR_EXTENSION = 6, EXTENSION_LOAD = 7, OUTPUT_RES = 8, WAIT_FOR_HASH = 9;
    ...
    // 3-Phase Schedule to prevent falling into Trapping Sets early:
    // Phase 1 (iter 0-9): Heavy offset=3, slows down initial belief propagation to avoid traps
    // Phase 2 (iter 10-29): Standard offset=2, steady convergence
    // Phase 3 (iter 30-49): Reduced offset=1, local correction
    wire [2:0] dynamic_offset = (iter_count < 10) ? 3'd3 : 
                                (iter_count < 30) ? 3'd2 : 3'd1;
    ...
    wire [shift_w-1:0] inv_shift_amt_d1 = (shift_val_d1 == 0) ? 0 : (Zc - shift_val_d1);
    wire [shift_w-1:0] current_shift_amt = (state == LAYER_WRITE) ? inv_shift_amt_d1 : shift_val_d1;
    ...
    barrel_shifter #(.Zc(Zc), .word_w(res_w), .shift_w(shift_w)) u_inv_shifter (
        .data_in(c2v_new_shifted), .shift_amt(inv_shift_amt_d1), .data_out(c2v_new_unshifted)
    );
    ...
    generate
        ...
        for(gi=0; gi<Zc; gi=gi+1) begin : gen_math
            ...
            wire [data_w-1:0] full_sat_max = (1 << (data_w-1)) - 1;
            // Dynamic Saturation (Architectural fix for Trapping Sets without BCH)
            // Phase 1 (iter 0-19): Cap LLR at 1/4 max to prevent false beliefs from growing too strong
            // Phase 2 (iter 20-39): Cap LLR at 1/2 max
            // Phase 3 (iter 40-49): Open floodgates to full max
            wire [data_w-1:0] sat_max = (iter_count < 20) ? (full_sat_max >> 2) :
                                        (iter_count < 40) ? (full_sat_max >> 1) : full_sat_max;
        ...
        if (rst) begin
            ...
            case (state)
                ...
                LAYER_READ: begin
                    ...
                    if (valid_read_1 && valid_conn_d1) begin
                ...
                LAYER_WRITE: begin
                    ...
                    if (col_count == 26) begin
                        ...
                        if (layer_count == max_layer - 1) begin
                            layer_count <= 0;
                ...
                CHECK: begin
                    // Nếu là Rate 1/2 thì phải đợi xuất Output ra rồi mới báo fail bằng Hash, để testbench có dữ liệu kiểm tra
                    if (!all_layers_parity_ok && current_code_rate != 2'b00) ir_fail_intr <= 1'b1;
                    block_count <= 0;
                    col_count <= 0;
                    llr_addr_r <= 0;
                end 
    ...
    always @(*) begin
        ...
        case (state)
            ...
            OUTPUT_RES: if (block_count == 25) next_state = WAIT_FOR_HASH;
            WAIT_FOR_HASH: begin
                if (hash_ok) next_state = IDLE;
                else if (hash_fail) begin
                    if (current_code_rate == 2'b00) next_state = IDLE;
                    else next_state = WAIT_FOR_EXTENSION;
                end
            end
        endcase
